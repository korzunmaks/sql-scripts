...
WHERE 
   Бух_Документ_ПриходнаяНакладная.date_time_iddoc >='20190716'
   AND Бух_Документ_ПриходнаяНакладная.date_time_iddoc <='20190716z'
	...
	AND Бух_Документ_ПриходнаяНакладная.IDD <> ''
 	 AND Бух_Документ_ПриходнаяНакладная.ФирмаIDD IN ('10015480660657321')
)
SELECT 
	ПриходныеНакладные.*
	, ISNULL(ПолученныеСчетфактурыIDD.СчетФактураIDD, '') AS ПолученныеСчетфактураIDD 
	, ISNULL(ПолученныеСчетфактурыIDD.СчетаФактураНомер, '') AS СчетаФактураНомер
	, ISNULL(ПолученныеСчетфактурыIDD.СчетаФактураДата, '17530101') AS СчетаФактураДата
FROM 
	ПриходныеНакладные
	OUTER APPLY
		(SELECT TOP (1) 
			ПолученныеСчетфактуры.IDD AS СчетФактураIDD
			, ПолученныеСчетфактуры.НомерСчетаФактуры AS СчетаФактураНомер
			, ПолученныеСчетфактуры.ДатаСчетаФактуры AS СчетаФактураДата
		FROM
